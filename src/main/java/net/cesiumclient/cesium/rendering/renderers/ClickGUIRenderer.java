...
//                Cesium.LOGGER.info("x1: " + category.x1 + " | x2: " + category.x2 + " | category: " + category.getTitle());
            ...
            if (category instanceof Search) {
                StringSetting stringSetting = Search.search;
                height1 += (generic.getStringHeight(stringSetting.name) / 2) + 18;
            }
            ...
            if (category instanceof Search) {
                height += 2;
                StringSetting stringSetting = Search.search;
                renderStringSetting(fs, category.x1, (int) (category.y2 + (height + (generic.getStringHeight(stringSetting.name) / 2) - 3)), category.x2, stringSetting, mouseX, mouseY, leftClicked);
                if (stringSetting.selected) {
                    if (keyCode != -1) {
                        stringSetting.onKey(keyCode, scanCode, modifiers);
                    }
                }
                height += (generic.getStringHeight(stringSetting.name) / 2) + 16;
